...
export class AgentNoSubmitError extends Error {}
...
export async function runSubmitAgent<T>(opts: {
  ...
  for (let turn = 1; turn <= maxTurns; turn++) {
    ...
    if (msg.stop_reason === "refusal") {
      throw new ResearchRefusalError(`model declined: ${msg.stop_details?.category ?? "unspecified"}`);
    }

    messages.push({ role: "assistant", content: msg.content });
    const done = () => ({ models: [...models], usage, costUsd, retrievedUrls: [...retrieved], turns: turn });
    ...
    if (submit) {
      ...
      if (parsed.success && problems.length === 0) return { output: parsed.data, ...done() };
      messages.push({
        role: "user",
  ...
  throw new AgentNoSubmitError(`agent did not submit a valid ${opts.submitTool.name} within ${maxTurns} turns`);
}
